...
async function startServer() {
  ...
  let DOUBAO_ENDPOINT = process.env.DOUBAO_ENDPOINT?.trim() || "https://ark.cn-beijing.volces.com/api/v3/chat/completions";
  const DOUBAO_MODEL_ID = process.env.DOUBAO_MODEL_ID?.trim() || "ep-m-20260305162457-9q2xm";
  ...
  async function callDoubao(prompt: string, systemInstruction: string, temperature = 0.7) {
    if (!DOUBAO_API_KEY || !DOUBAO_MODEL_ID) {
      throw new Error("豆包 API 配置缺失，请设置 DOUBAO_API_KEY 和 DOUBAO_MODEL_ID");
    }
