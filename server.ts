...
import { assertValidSimulationResult } from "./src/services/resultValidation";
...
async function startServer() {
  ...
  // 2. If API Key looks like an endpoint ID
    ...
    DOUBAO_API_KEY = (DOUBAO_MODEL_ID.includes("-") && DOUBAO_MODEL_ID.length === 36) ? DOUBAO_MODEL_ID : undefined;
  ...
  app.post("/api/ai/simulation", async (req, res) => {
    ...
    if (cached) {
      try {
        const cachedResult = assertValidSimulationResult(cached, "Cached simulation");
        console.log(`[AI Simulation] Cache hit! Returning stored response.`);
        return res.json(cachedResult);
      } catch (error) {
        console.error("[AI Simulation] Ignoring invalid cached response:", error);
      }
    }
    ...
    try {
      ...
      if (PRESET_RESPONSES[input]) {
        ...
        return res.json(assertValidSimulationResult(PRESET_RESPONSES[input], "Preset simulation"));
      }
      ...
      finalResult = assertValidSimulationResult(finalResult, "AI simulation");
