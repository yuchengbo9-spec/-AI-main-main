...
import { assertSimulationResult } from "./src/services/resultValidation";
...
async function startServer() {
  ...
  app.post("/api/ai/simulation", async (req, res) => {
    ...
    if (cached) {
      try {
        const cachedResult = assertSimulationResult(cached, "Cached simulation response");
        console.log(`[AI Simulation] Cache hit! Returning stored response.`);
        return res.json(cachedResult);
      } catch (validationError) {
        console.error("[AI Simulation] Ignoring invalid cached response:", validationError);
      }
    }
    ...
    try {
      ...
      if (PRESET_RESPONSES[input]) {
        const presetResult = assertSimulationResult(PRESET_RESPONSES[input], `Preset response for "${input}"`);
        console.log(`[AI Simulation] Hit Preset Response for: ${input}`);
        // Simulate a small delay for realistic feel
        await new Promise(resolve => setTimeout(resolve, 800));
        return res.json(presetResult);
      }
      ...
      finalResult = assertSimulationResult(finalResult, "AI simulation response");
